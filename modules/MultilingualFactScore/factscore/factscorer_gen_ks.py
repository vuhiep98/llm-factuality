...
class FactScorer(object):
    ...
    def __init__(self,
        ...
        self.ks_generator = OpenAIModel("ChatGPT-gpt4",
                                cache_file=os.path.join(cache_dir, "GPT-4-ks-generator.pkl"),
                                key_path=openai_key)
    ...
    def save_cache(self):
        ...
        for k, v in self.retrieval.items():
            v.save_cache()
        if self.ks_generator:
            self.ks_generator.save_cache()
            ...
            if self.lm:
                if "retrieval" in self.model_name:
                    ...
                    if passages == None or len(passages) == 0:
                        ...
                        continue
                    gen_question_prompt = f'''
                    Make a question about {topic} for the below sentence.

                    Sentence: {atom.strip()}
                    '''
                    ks_question = self.ks_generator.generate(gen_question_prompt)[0].strip()
                    print("ks_question:", ks_question)
                    gen_knowledge_prompt = f'''
                    Generate background information to answer the below question about {topic}.

                    Question: {ks_question}
                    '''
                    gen_ks = self.ks_generator.generate(gen_knowledge_prompt)[0].strip()
                    print("gen_ks:", gen_ks)
                    if "bloomz" in self.model_name:
                        ...
                        context = ""
                        for psg_idx, psg in enumerate(reversed(passages)):
                            if psg_idx == langs_avai[self.lang]//2:
                                context += "Title: {}\nText: {}\n\n".format(psg["title"], gen_ks.replace("<s>", "").replace("</s>", ""))
...
if __name__ == '__main__':
    ...
    if args.use_atomic_facts:
        with open(args.input_path.replace(".jsonl", f"{args.lang}_"+args.model_name+f"_factscore_output_provided_facts_gen_ks.json"), 'w') as f:
