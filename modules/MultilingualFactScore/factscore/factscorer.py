...
langs_avai = {"es": 7, "bn": 8, "ar":8, "en":5, "fr": 5, "vi":5, "zh-cn":5, "ru":5, "hi":8}
...
class FactScorer(object):
    ...
    def __init__(self,
        ...
        if "llama" in model_name:
            self.lm = CLM("llama3.1-8B-Instruct",
    ...
    def get_score(self,
        ...
        if knowledge_source not in self.retrieval:
            self.register_knowledge_source(
                knowledge_source, 
                data_path=os.path.join(self.data_dir, f"{knowledge_source}.jsonl"), 
                db_path=os.path.join(self.data_dir, f"{knowledge_source}_100.db")
            )
...
if __name__ == '__main__':
    ...
    if args.use_atomic_facts:
        output_file = os.path.join(output_path, f"{llm_name}_factscore_output_provided_facts.json")
    else:
        output_file = os.path.join(output_path, f"{llm_name}_factscore_output_gen_facts.json")
    json.dump(out, open(output_file, "w"), indent=4, ensure_ascii=False)
