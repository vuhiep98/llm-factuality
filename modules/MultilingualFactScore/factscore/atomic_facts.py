...
class AtomicFactGenerator(object):
    def __init__(self, model_name, demon_dir, cache_dir, model_dir, lang = "en"):
        self.nlp = spacy.load("en_core_web_sm")
        self.is_bio = True
        self.demon_path = os.path.join(demon_dir, "demons_"+ lang + ".json" if self.is_bio else "demons_complex.json")
        # self.demon_path = os.path.join(demon_dir, "demons.json")
        self.lang = lang
        ...
        self.lm = CLM(model_name=self.model_name,
                      model_dir=self.model_dir,
                      cache_file=os.path.join(cache_dir, f"af-{self.model_name}.pkl"))
        # self.lm.load_model()
        # self.lm = PeftModel.from_pretrained(self.lm.model, "Rashik24/Mistral-Instruct-Bangla")
        # self.lm = OpenAIModel("ChatGPT-gpt4", cache_file="~/projects/factscore/GPT-4-extract-facts.pkl", key_path=key_path)
        # self.lm = OpenAIModel("ChatGPT", cache_file="~/projects/factscore/ChatGPT-extract-facts-translated.pkl", key_path=key_path)
        # self.lm = GeminiModel("Gemini", cache_file=os.path.join(cache_dir, "Gemini-pro-fact-extract.pkl"), key_path=key_path)
        # get the demos
...
def main():
    ...
    generator = AtomicFactGenerator(model_dir="~/projects/factscore", cache_dir="~/projects/factscore", key_path="~/projects/open_ai_key.txt", demon_dir="~/projects/factscore/demos", gpt3_cache_file="~/projects/factscore/InstructGPT.pkl")
