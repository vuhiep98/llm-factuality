...
class FactScorer(object):
    ...
    def __init__(self,
        ...
        name_map_path = f"~/FActScore/data/to_evaluate/trans2ori/{lang}_name.json"
        db_map_path = f"~/FActScore/data/to_evaluate/trans2ori/{lang}_db.json"
        fact_map_path = f"~/FActScore/data/to_evaluate/trans2ori/{lang}_{evaluated_model}_label_by_GPT-4.json"
        with open(name_map_path) as f:
            for line in f:
                self.name_map = json.loads(line)
        with open(db_map_path) as f:
            for line in f:
                self.db_map = json.loads(line)
        with open(fact_map_path) as f:
            for line in f:
                self.fact_map = json.loads(line)
    ...
    def register_knowledge_source(self, name="enwiki-20230401", db_path=None, data_path=None):
        ...
        cache_path = os.path.join(self.cache_dir, f"retrieval_n-{name}.json")
        embed_cache_path = os.path.join(self.cache_dir, f"retrieval_n-{name}.pkl")
        ...
        for atom in atomic_facts:
            ...
            if self.lm:
                if "retrieval" in self.model_name:
                    passages = self.retrieval[knowledge_source].get_passages(topic, atom, k=langs_avai[self.lang])
                    new_passages = []
                    if passages == None or len(passages) == 0:
                        logging.critical(f"passages == None {topic}")
                        decisions.append({"atom": atom, "is_supported": bool(False)})
                        continue
                    for psg in passages:
                        assert psg["text"].replace("<s>", "").replace("</s>", "").strip() in self.db_map, "not in db map"
                        assert psg["title"] in self.name_map, "not in name map"
                        new_passages.append({"title": self.name_map[psg["title"]],"text": self.db_map[psg["text"].replace("<s>", "").replace("</s>", "").strip()]})
                    passages = new_passages.copy()
                    ...
                    if "bloomz" in self.model_name:
                        ...
                        assert topic in self.name_map, "not in fact map"
                        definition = "Answer the question about {} based on the given context.\n\n".format(self.name_map[topic.strip()])
                        context = ""
                        for psg_idx, psg in enumerate(reversed(passages)):
                        ...
                        if not definition[-1] in string.punctuation:
                            definition += "."
                        assert atom in self.fact_map, "not in fact map"
                        prompt = "{}\n\nInput: {} True or False? \nOutput: ".format(definition.strip(), self.fact_map[atom.strip()])
                else:
...
if __name__ == '__main__':
    ...
    if args.use_atomic_facts:
        with open(args.input_path.replace(".jsonl", f"{args.lang}_"+args.model_name+f"_factscore_output_provided_facts_trans_map.json"), 'w') as f:
