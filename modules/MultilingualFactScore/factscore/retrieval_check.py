...
class DocDB(object):
    ...
    def get_text_from_title(self, title):
        ...
        # print("FIRST:", results[0][0].split(SPECIAL_SEPARATOR)[0], len(results[0][0].split(SPECIAL_SEPARATOR)[0]))
        results1 = [{"title": title, "text": para} for para in results[0][0].split(SPECIAL_SEPARATOR)]
        # print("results",results[0][0].split(SPECIAL_SEPARATOR))
        # print("################")
        assert len(results1)>0, f"`topic` in your data ({title}) is likely to be not a valid title in the DB."
        return results1

class Retrieval(object):

    def __init__(self, db, cache_path, embed_cache_path,
                retrieval_type="paraphrase-multilingual-MiniLM-L12-v2", batch_size=None):
