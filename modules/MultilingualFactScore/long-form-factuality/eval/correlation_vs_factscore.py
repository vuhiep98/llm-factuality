...
_EVAL_IN_PARALLEL = flags.DEFINE_boolean(
    'eval_in_parallel', default=False, help='Whether to evaluate in parallel.'
)
...
_FACTSCORE_DATA_FOLDER = os.path.join(
    shared_config.root_dir, 'third_party/entities_with_human_annotated/bn/'
)
_DATE_AND_TIME = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
# _HUMAN_ATOMIC_FACTS = 'human-atomic-facts-gt'
...
def save_results(
  ...
  out_name = f'{_DATE_AND_TIME}-{_FACTSCORE_DATA_FOLDER.replace("/", "_")}-correlation_vs_factscore.json'
  out_path = os.path.join(out_folder, out_name)
  ...
  for do_step, step_name in [
    ...
    all_results[step_name] = {
        **response_level_scores,
        ...
        'folder_path': _FACTSCORE_DATA_FOLDER,
    }
